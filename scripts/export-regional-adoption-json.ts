/**
 * Export: Regional Peppol adoption data to JSON
 * Generates static JSON for peppolcheck's Regional Adoption Map
 *
 * Usage:
 *   npx tsx scripts/export-regional-adoption-json.ts [output-path]
 *
 * Args:
 *   output-path: Path to output JSON file (default: ../peppolcheck/data/regional-adoption-data.json)
 */
...
interface RegionalAdoptionData {
  snapshotDate: string
...
async function exportRegionalAdoption(outputPath: string) {
  ...
  try {
    ...
    console.log('Querying regional adoption data...')

    // Main query with establishment fallback and postal-to-arrondissement mapping
    const postalMappingPath = resolve(__dirname, '../data/postal-mapping.json')
    const query = `
      WITH
      -- Load postal code to arrondissement mapping from local JSON file
      -- Source: codes-ins-nis-postaux-belgique.geojson (official Belgian data)
      -- Contains all 43 arrondissements including La Louvière (58)
      -- Mouscron (54) is merged into Tournai-Mouscron (57) in the source file
      postal_mapping AS (
        SELECT DISTINCT
          postal_code,
          arr_nis AS arrondissement_nis,
          arr_nl AS arrondissement_name_nl,
          arr_fr AS arrondissement_name_fr
      ...
      entity_arrondissement AS (
        SELECT
          ez.entity_number,
          ez.zipcode,
          pm.arrondissement_nis,
          pm.arrondissement_name_nl,
          pm.arrondissement_name_fr
        FROM entity_zipcode ez
        LEFT JOIN postal_mapping pm ON TRIM(CAST(ez.zipcode AS VARCHAR)) = pm.postal_code
      )

      -- Final aggregation by arrondissement (group by NIS code only, use MAX for name)
      SELECT
        ea.arrondissement_nis,
        MAX(ea.arrondissement_name_nl) AS arrondissement_name_nl,
        MAX(ea.arrondissement_name_fr) AS arrondissement_name_fr,
        COUNT(DISTINCT ea.entity_number) AS total_entities,
        COUNT(DISTINCT CASE WHEN p.enterprise_number IS NOT NULL THEN ea.entity_number END) AS peppol_registered,
        ROUND(100.0 * COUNT(DISTINCT CASE WHEN p.enterprise_number IS NOT NULL THEN ea.entity_number END) /
              NULLIF(COUNT(DISTINCT ea.entity_number), 0), 1) AS adoption_pct
      FROM entity_arrondissement ea
      LEFT JOIN peppol_companies p ON ea.entity_number = p.enterprise_number
      WHERE ea.arrondissement_nis IS NOT NULL
      GROUP BY ea.arrondissement_nis
      ORDER BY ea.arrondissement_nis
    `
    ...
    for (const chunk of await result.fetchAllChunks()) {
      for (const row of chunk.getRows()) {
        // NIS code is already in 5-digit format from the mapping
        const nisCode = String(row[0])
        arrondissements.push({
          nisCode,
          nameNl: String(row[1]),
          nameFr: String(row[2]),
          totalEntities: Number(row[3]),
          peppolRegistered: Number(row[4]),
          adoptionPct: Number(row[5])
        })
      }
    }

    console.log(`Found ${arrondissements.length} arrondissements`)

    // Query for coverage stats
    const coverageQuery = `
      WITH
      vat_entities AS (
        SELECT DISTINCT entity_number
    ...
    // Build final output
    const output: RegionalAdoptionData = {
    ...
    console.log(`Arrondissements: ${arrondissements.length}`)
    ...
    // Summary stats
    const totalEntities = arrondissements.reduce((sum, a) => sum + a.totalEntities, 0)
    const totalPeppol = arrondissements.reduce((sum, a) => sum + a.peppolRegistered, 0)
    ...
    // Show top/bottom arrondissements
    const sorted = [...arrondissements].sort((a, b) => b.adoptionPct - a.adoptionPct)
    console.log('\nTop 5 arrondissements:')
    for (const arr of sorted.slice(0, 5)) {
      console.log(`  ${arr.nameNl}: ${arr.adoptionPct}% (${arr.peppolRegistered.toLocaleString()}/${arr.totalEntities.toLocaleString()})`)
    }
    console.log('\nBottom 5 arrondissements:')
    for (const arr of sorted.slice(-5).reverse()) {
      console.log(`  ${arr.nameNl}: ${arr.adoptionPct}% (${arr.peppolRegistered.toLocaleString()}/${arr.totalEntities.toLocaleString()})`)
    }
...
const outputPath = process.argv[2] || resolve(__dirname, '../../peppolcheck/data/regional-adoption-data.json')
exportRegionalAdoption(outputPath).catch(console.error)
